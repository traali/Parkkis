...
async function analyze() {
    const signs = await fs.readJson(path.join(process.cwd(), '.cache/signs.json'));
    const total = signs.features.length;
    const withDate = signs.features.filter(f => f.properties.ens_vo_pv && f.properties.ens_vo_pv.trim() !== '').length;
    const withModDate = signs.features.filter(f => f.properties.muokkauspv && f.properties.muokkauspv.trim() !== '').length;
    
    console.log(`Total Signs: ${total}`);
    console.log(`With ens_vo_pv: ${withDate}`);
    console.log(`With muokkauspv: ${withModDate}`);
    
    const modDates = signs.features.map(f => {
    ...
    console.log(`muokkauspv Range: ${minDate.toISOString()} to ${maxDate.toISOString()}`);
