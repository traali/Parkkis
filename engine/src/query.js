...
const violationsPath = path
  .join(OUTPUT_DIR, "violations.parquet")
  .replace(/\\/g, "/");
...
async function main() {
  ...
  const slotsData = await runQuery(
    `SELECT * EXCLUDE (geom) FROM '${slotsPath}' LIMIT 3`,
  );
  console.log(slotsData);

  console.log("--- Fines Schema ---");
  const finesSchema = await runQuery(
    `DESCRIBE SELECT * FROM '${violationsPath}'`,
  );
  console.log(finesSchema);

  console.log("--- Sample Fines Data ---");
  const finesData = await runQuery(
    `SELECT * EXCLUDE (geom) FROM '${violationsPath}' LIMIT 3`,
  );
